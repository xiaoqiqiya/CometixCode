...
fn unix() {
    ...
    for (input, right) in cases {
        ...
        assert!(
            matches!(strict, Cow::Borrowed(_)),
            "strict result should borrow: {input:#?}"
        );
        assert!(
            matches!(lossy, Cow::Borrowed(_)),
            "lossy result should borrow: {input:#?}"
        );
...
fn windows() {
    ...
    for (input, right) in cases {
        ...
        let lossy = input.to_slash_lossy();
        assert!(
            matches!(strict, Cow::Owned(_)),
            "strict result should own: {input:#?}"
        );
        assert!(
            matches!(lossy, Cow::Owned(_)),
            "lossy result should own: {input:#?}"
        );
        assert_eq!(strict, right, "case: {input:#?}");
        assert_eq!(lossy, right, "case: {input:#?}");
    }
