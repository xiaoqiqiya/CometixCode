...
fn unix_upward_dirty_and_invalid_results_remain_owned() {
    ...
    let target = PathBuf::from(OsString::from_vec(
        b"/workspace/rolldown/invalid-\x80.js".to_vec(),
    ));
...
#[test]
fn windows_descendants_match_values_and_ordinary_paths_borrow() {
    for (target, base, expected) in [
        (
            r"C:\workspace\rolldown\src\index.js",
            r"C:\workspace\rolldown",
            r"src\index.js",
        ),
        (
            r"C:\workspace\rolldown/src\index.js",
        ...
        let target = Path::new(target);
        let result = target.relative(base);
        assert_eq!(result.as_os_str().as_encoded_bytes(), expected.as_bytes());
        // Borrowing is an upstream optimization contract, not a JS observable.
        // Namespace paths now use the shared lexical owner and may allocate.
        if !super::super::windows_lexical::is_namespace(target) {
            assert!(matches!(result, Cow::Borrowed(_)));
            assert_borrows_from_target(target, &result);
        }
    }
}
...
fn windows_owned_results_preserve_relative_semantics() {
    for (target, base) in [
        (
            "C:/workspace/rolldown/src/index.js",
            "C:/workspace/rolldown",
        ),
        (
            r"C:\workspace\rolldown\src/index.js",
            r"C:\workspace\rolldown",
    ...
    // Node removes the earlier dot; the resulting clean suffix may still borrow.
    assert_eq!(
        Path::new(r"C:\workspace\rolldown\.\src\index.js")
            .relative(r"C:\workspace\rolldown")
            .as_os_str(),
        r"src\index.js"
    );
}
