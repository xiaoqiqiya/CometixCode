...
use super::test_utils::{assert_eq_str, p};
use std::borrow::Cow;
...
fn unix_absolutize_with() {
    ...
    // Complex paths
    assert_eq_str!(
        "./foo/../bar/./baz".absolutize_with("/root"),
        "/root/bar/baz"
    );
...
fn unix_absolutize_with_borrowed_and_owned_cwd_arguments() {
    assert_eq_str!(
        "documents".absolutize_with("/home/user".as_path()),
        "/home/user/documents"
    );
...
fn windows_absolutize_with() {
    ...
    // With dots in paths
    assert_eq_str!(
        ".\\a\\.\\b\\..\\c".absolutize_with("C:\\base"),
        "C:\\base\\a\\c"
    );
    assert_eq_str!(
        "..\\a\\..\\b".absolutize_with("C:\\base\\dir"),
        "C:\\base\\b"
    );
    ...
    assert_eq_str!("c:file".absolutize_with("C:\\base"), "c:\\base\\file");
    assert_eq_str!(r"C:file".absolutize_with(r"\\?\C:\base\D:dir"), r"C:file",);
    assert_eq_str!(r"C:file".absolutize_with(r"\\?\C:\base\foo/bar"), r"C:file",);
    assert_eq_str!(r"C:file".absolutize_with(r"\\?\c:\base"), r"C:file");
...
fn windows_absolutize_with_unc_paths() {
    // UNC path tests
    assert_eq_str!(
        "file".absolutize_with("\\\\server\\share"),
        "\\\\server\\share\\file"
    );
    assert_eq_str!(
    ...
    assert_eq_str!(
        "\\\\other\\share".absolutize_with("\\\\server\\share"),
        "\\\\other\\share\\"
    );
}
...
fn windows_absolutize_with_mixed_separators() {
    ...
    assert_eq_str!(
        "sub/folder".absolutize_with("C:\\base"),
        "C:\\base\\sub\\folder"
    );
    assert_eq_str!(
        "./sub\\folder".absolutize_with("C:/base"),
...
fn absolutize_with_does_not_validate_an_unused_cwd() {
    #[cfg(target_family = "unix")]
    assert_eq_str!(
        "/already/absolute/".absolutize_with("relative/cwd"),
        "/already/absolute"
    );
    #[cfg(target_family = "windows")]
    assert_eq_str!(
        r"C:\already\absolute\".absolutize_with("relative/cwd"),
        r"C:\already\absolute"
    );
}
...
fn windows_clean_absolute_paths_return_borrowed_cow() {
    ...
    for path in [r"C:\foo\bar", r"C:\", r"\\server\share\dir"] {
        assert!(
...
fn windows_dirty_or_relative_paths_return_owned_cow() {
    ...
    assert!(
        matches!(
            p!(drive_relative).absolutize_with("C:\\base"),
            Cow::Owned(_)
        ),
