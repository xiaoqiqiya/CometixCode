...
fn windows_lexical_relative_matrix_has_exact_stable_results() {
    for (base, target, expected) in [
        (
            r"dist\chunks",
            r"dist\assets\index.js",
            r"..\assets\index.js",
        ),
        (
            r"dist\.\chunks\..\chunks",
            r".\dist\assets\.\temp\..\index.js",
            r"..\assets\index.js",
        ),
        (
            r"..\..\dist\chunks",
    ...
    for group in SAFE_GROUPS {
        for target in *group {
            for base in *group {
                ...
                for cwd in CWDS {
                    let expected =
                        oracle_with_cwd(Path::new(target), Path::new(base), Path::new(cwd));
                    assert_eq!(
                        actual.as_os_str(),
                        expected.as_os_str(),
