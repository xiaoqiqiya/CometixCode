...
#[cfg(target_family = "windows")]
#[test]
fn windows_namespace_normalization_matches_official_slash_semantics() {
    for (path, expected) in [
        (r"\\?\C:\foo/", r"\\?\C:\foo\"),
        (r"\\?\UNC\server\share\foo/", r"\\?\UNC\server\share\foo\"),
        (r"\\?\Volume{abc}\foo/", r"\\?\Volume{abc}\foo\"),
    ] {
...
fn unix_clean_current_directory_spellings_return_borrowed_cow() {
    for (path, expected) in [("", "."), (".", "."), ("./", "./")] {
        ...
        assert!(
            matches!(normalized, Cow::Borrowed(_)),
            "expected borrowed Cow for {path:?}"
        );
...
fn unix_dirty_paths_return_owned_cow() {
    for (path, expected) in [
        ...
        assert!(
            ...
            "expected owned Cow for dirty path {:?}",
            path,
        );
        assert_eq_str!(normalized, expected);
    }
...
fn unix_dirty_leading_parent_paths_return_owned_cow() {
    for (path, expected) in [
        ...
        assert!(
            matches!(normalized, Cow::Owned(_)),
            "expected owned Cow for dirty path {path:?}"
        );
        assert_eq!(normalized.as_os_str(), p!(expected).as_os_str());
    }
}
