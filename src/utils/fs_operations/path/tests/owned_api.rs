...
fn owned_apis_preserve_and_replace_invalid_unix_encoding() {
    ...
    let returned = input
        .clone()
        .try_into_slash()
        .expect_err("invalid Unix encoding must be returned unchanged");
    assert_eq!(
        returned.as_os_str().as_bytes(),
...
fn owned_apis_preserve_and_replace_invalid_windows_encoding() {
    ...
    assert_eq!(
        returned.as_os_str().encode_wide().collect::<Vec<_>>(),
        input_wide
    );
    assert_eq!(
        input.into_slash_lossy(),
        "C:/workspace/invalid-\u{fffd}/./file"
    );
}
