...
use super::test_utils::assert_eq_str;
...
fn windows_absolute_remainders_are_normalized() {
    for (target, base, expected) in [
        ...
        (
            r"C:\workspace\base\src\\index.js",
            r"C:\workspace\base",
            r"src\index.js",
        ),
        (
...
fn assert_same_windows_root_ignoring_case(base: &str, target: &str, expected: &str) {
    ...
    assert!(
        rejoined
            .to_str()
            .unwrap()
            .eq_ignore_ascii_case(target.to_str().unwrap()),
...
fn windows_root_matrix() {
    // Node relative can traverse shares on the same UNC server lexically.
    ...
    assert_eq_str!(
        Path::new(r"\\server\other\packages\app\index.js").relative(r"\\server\share\dist\chunks"),
        r"..\..\..\other\packages\app\index.js"
    );
    ...
    // JS namespace relative treats server/share as lexical components.
    assert_same_windows_root(
    ...
    assert_eq_str!(
        Path::new(r"\\?\UNC\server\other\packages\app\index.js")
            .relative(r"\\?\UNC\server\share\dist\chunks"),
        r"..\..\..\other\packages\app\index.js"
    );
    assert_eq_str!(
        Path::new(r"\\?\UNC\other\share\packages\app\index.js")
            .relative(r"\\?\UNC\server\share\dist\chunks"),
        r"..\..\..\..\other\share\packages\app\index.js"
    );
    ...
    assert_eq_str!(
        Path::new(r"\\?\D:\workspace\rolldown\packages\app\index.js")
            .relative(r"\\?\C:\workspace\rolldown\dist\chunks"),
        r"..\..\..\..\..\D:\workspace\rolldown\packages\app\index.js"
    );
    assert_different_windows_roots(
        r"C:\workspace\rolldown\dist\chunks",
    ...
    assert_eq_str!(
        Path::new(r"\\.\MAILSLOT\rolldown\packages\app\index.js")
            .relative(r"\\.\PIPE\rolldown\dist\chunks"),
...
#[cfg(target_family = "windows")]
#[test]
fn windows_relative_preserves_node_colon_and_namespace_components() {
    for (base, target) in [
        (r"\\?\C:\base", r"\\?\C:\base\foo/bar"),
        (
            r"\\?\UNC\server\share\base",
            r"\\?\UNC\server\share\base\foo/bar",
        ),
        (r"\\?\Volume{abc}\base", r"\\?\Volume{abc}\base\foo/bar"),
    ] {
        assert_eq_str!(Path::new(target).relative(base), r"foo\bar");
    }
    for (base, target) in [(r"C:\base", r"C:\base\C:foo")] {
        let relative = Path::new(target).relative(base);
        assert_eq_str!(relative, "C:foo", "target {target:?}, base {base:?}");
    }

    // JS treats both slash forms as separators, including namespace paths.
    assert_eq_str!(
        Path::new(r"\\?\C:\base\other").relative(r"\\?\C:\base\foo/bar"),
        r"..\..\other",
    );

    assert_eq_str!(
        Path::new(r"C:\base\dir\C:foo").relative(r"C:\base"),
        r"dir\C:foo"
    );
    assert_eq_str!(
        Path::new(r"C:\base\C:foo").relative(r"C:\base\other"),
        r"..\C:foo"
    );

    for (base, target, expected) in [
        (r"dist", r"dist\C:foo", r"C:foo"),
        (r"C:.", r"C:C:foo", r"C:foo"),
    ] {
    ...
    assert_eq_str!(
        Path::new(r"dir\C:foo").relative_with("", "not/absolute"),
        r"dir\C:foo",
    );
    assert_eq_str!(
        Path::new(r".\C:foo").relative_with("other", "not/absolute"),
        r"..\C:foo",
    );
}
...
#[cfg(target_family = "windows")]
#[test]
fn windows_relative_matches_official_unicode_case_expansion() {
    assert_eq_str!(Path::new(r"c:\i̇\b").relative(r"C:\İ\a"), r"..\b");
    assert_eq_str!(Path::new(r"İ\a").relative_with(r"i̇\b", r"C:\cwd"), r"..\a");
}
