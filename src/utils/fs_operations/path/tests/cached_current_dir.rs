...
#[test]
fn current_directory_changes_are_observed() {
    ...
    let expected_base = &third;
    assert_eq!(
        Path::new("entry.js").absolutize(),
        expected_base.join("entry.js")
    );
    ...
    #[cfg(target_family = "windows")]
    {
        ...
        let oracle =
            std::path::absolute(&drive_relative).expect("resolve the drive-relative oracle");
        assert_eq!(
