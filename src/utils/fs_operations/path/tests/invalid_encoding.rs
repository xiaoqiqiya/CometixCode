...
mod unix {
    ...
    fn lexical_relative_common_prefix_compares_invalid_encoding_exactly() {
        ...
        assert_eq!(
            path(b"segment-\x80").to_string_lossy(),
            path(b"segment-\x81").to_string_lossy()
        );
        let distinct_base = path(b"../../dist/segment-\x80/chunks");
        let distinct_target = path(b"../../dist/segment-\x81/assets");
        assert_bytes(
            &distinct_target.relative(&distinct_base),
            b"../../segment-\x81/assets",
        );
    }
...
mod windows {
    ...
    // Native surrogate preservation is a Rust boundary contract; it is not a
    // claim that Bun retains lone UTF-16 surrogates in JS path strings.
    ...
    fn assert_normalizes_exactly_and_is_idempotent(
        ...
        let once = invalid_path(input_prefix, input_suffix)
            .normalize()
            .into_owned();
    ...
    #[test]
    fn relative_fallback_preserves_native_encoding_across_windows_roots() {
        assert_relative_cases(&[
            (
                "different disk",
                r"c:\base\segment-",
    ...
    fn lexical_relative_common_prefix_compares_invalid_encoding_exactly() {
        ...
        let expected =
            wide_with_invalid_unit(r"..\..\segment-", DISTINCT_HIGH_SURROGATE, r"\assets");
    ...
    #[test]
    fn lowercase_unc_marker_matches_official_namespace_relative() {
        let target = Path::new(r"\\?\unc\server\share\file");
        let base = Path::new(r"\\?\UNC\server\share");
        let relative = target.relative(base);
        assert_eq!(relative.as_os_str(), "file");
    }
